...
class m_search_report extends CI_Model{
    ...
    public function search_send_report($level_institution_id,$level_institution,$typeSelect,$typeSelect,$from_number_of_run,$to_number_of_run,$startDate,$endDate,$subject,$to_receive,$number_of_run_1,$number_of_run_2,$number_of_run_3,$number_of_run_4,$number_of_run_5,$number_of_run_6,$number_of_run_7,$number_of_run_8,$form_send){
        $srtWhere = "";
        $chk = false;
       ...
       if ($from_number_of_run != "" && $to_number_of_run != ""){
        ...
        if ($form_send != ""){
            if ($srtWhere != ""){$srtWhere = $srtWhere." and ";}
            $srtWhere = $srtWhere." rcn.from like '%".trim($form_send)."%'";
        }
    ...
    public function show_booksend_detail($id = "",$level_institution_id = "",$level_institution =""){
     ...
     public function search_recieve_report($department_id,$level_institution_id,$level_institution,$typeSelect,$from_number_of_run,$to_number_of_run,$startDate,$endDate,$subject,$to_receive,$number_of_run_1,$number_of_run_2,$number_of_run_3,$number_of_run_4,$number_of_run_5,$number_of_run_6,$number_of_run_7,$number_of_run_8,/*$from_send_id,$from_send_level*/$form_send){
        $srtWhere = "";
        $chk = false;
       ...
       if (trim($from_number_of_run) != "" && trim($to_number_of_run) != ""){
        ...
        if ($department_id != "" ){
           if ($srtWhere != ""){$srtWhere = $srtWhere." and ";}
            $srtWhere = $srtWhere." rrd.document_no like '%".$department_id."%'";
        }
        //วันที่
        if ($startDate != "" && $endDate != ""){
            if ($srtWhere != ""){$srtWhere = $srtWhere." and ";}
